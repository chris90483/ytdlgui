...
from recorder.radiorecorder import RadioRecorder
...
def toggle_record(app_state_arg):
    if radio_recorder.is_recording:
        radio_recorder.stop_recording()
    else:
        radio_recorder.record(app_state_arg.recorder_state['active_station_name'], {"now_recording": now_recording_text})
...
recorder_input_area.grid(row=0, column=0)

recorder_status_area = Frame(recorder)
recorder_status_area.grid(row=1, column=0)
...
record_button = Button(recorder_input_area, text="Opnemen", command=lambda: toggle_record(app_state))
record_button.grid(row=0, column=2)

now_recording_text = StringVar(recorder_status_area)
now_recording_text.set("Niet aan het opnemen.")
now_recording_label = Label(recorder_status_area, textvariable=now_recording_text)
now_recording_label.grid(row=1, column=0, sticky=W)
